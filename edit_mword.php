...
function edit_mword_do_operation($translation, $wid) {
    ...
    if (mb_strtolower($text, 'UTF-8') == $textlc) {
        ...
        if ($_REQUEST['op'] == 'Save') {
            $message = edit_mword_do_insert($translation);
            $wid = get_last_key();
        } // $_REQUEST['op'] == 'Save'
        ...
        // UPDATE
            ...
            $message = edit_mword_do_update($translation);
            $wid = $_REQUEST["WoID"];
        } // $_REQUEST['op'] != 'Save'
...
function edit_mword_do_insert($translation) {
    global $tbpref;
...
function edit_mword_do_update($translation) {
    global $tbpref;
...
<script type="text/javascript">
    ...
    if ($new) {
        edit_mword_new($wid, $lang, $termlc, $term, $scrdir);
    }
...
function edit_mword_new($wid, $lang, $termlc, $term, $scrdir) {
    global $tbpref;
