...
$.post('inc/ajax.php',request,function(d){if(d==''){alert(failure)}else{do_ajax_edit_impr_text(pagepos,d)}})}
function changeTableTestStatus(wordid,up){$.post('inc/ajax.php',{action:"increase_term_status",action_type:"increase_term_status",id:wordid,status_up:(up?1:0)},function(data){if(data!=''){$('#STAT'+wordid).html(data)}})}
...
function do_ajax_update_media_select(){$('#mediaSelectErrorMessage').css("display","none");$('#mediaselect select').css("display","none");$('#mediaSelectLoadingImg').css("display","inherit");$.get('inc/ajax.php',{action:"query",action_type:"media_paths"},media_select_receive_data,"json")}
...
return outElement}
function do_ajax_show_sentences(lang,word,ctl,woid){$('#exsent-interactable').css("display","none");$('#exsent-waiting').css("display","inherit");$.get('inc/ajax.php',{action:"query",action_type:"example_sentences",lid:lang,word_lc:word,wid:woid},function(data){$('#exsent-waiting').css("display","none");$('#exsent-sentences').css("display","inherit");const new_element=display_example_sentences(data,ctl);$('#exsent-sentences').append(new_element)},"json")}
...
function getPhoneticText(text,lang){let phoneticText;$.ajax({url:'inc/ajax.php',async:!1,data:{action:"query",action_type:"phonetic_reading",text:text,lang:lang},type:"GET",}).done(function(data){phoneticText=data});return phoneticText}
async function getPhoneticTextAsync(text,lang){return $.get('inc/ajax.php',data={action:"query",action_type:"phonetic_reading",text:text,lang:lang},)}
