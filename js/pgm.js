...
function do_ajax_update_media_select(){$('#mediaSelectErrorMessage').css("display","none");$('#mediaselect select').css("display","none");$('#mediaSelectLoadingImg').css("display","inherit");$.getJSON('api.php/v1/media-path',{},media_select_receive_data)}
...
function saveReadingPosition(text_id,position){$.ajax({type:"POST",url:'api.php/v1/texts/'+text_id+'/reading-position',data:{position:position},async:!1})}
function saveCurrentPosition(){let pos=0;const top_pos=$(window).scrollTop()-$('.wsty').not('.hide').eq(0).height();$('.wsty').not('.hide').each(function(){if($(this).offset().top>=top_pos){pos=$(this).attr('data_pos');return!1}});saveReadingPosition(text_id,pos)}
