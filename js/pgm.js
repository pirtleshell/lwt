...
function changeTableTestStatus(wordid,up){const status_change=up?'up':'down';const wid=parseInt(wordid,10);$.post('api.php/v1/terms/'+wid+'/status/'+status_change,{action:"term_status",action_type:"increment",wid:wid,status_up:(up?1:0)},function(data){if(data==""||"error" in data){return}
...
function do_ajax_update_media_select(){$('#mediaSelectErrorMessage').css("display","none");$('#mediaselect select').css("display","none");$('#mediaSelectLoadingImg').css("display","inherit");$.getJSON('api.php/v1/media-path',{action:"query",action_type:"media_paths"},media_select_receive_data)}
...
return outElement}
function do_ajax_show_sentences(lang,word,ctl,woid){$('#exsent-interactable').css("display","none");$('#exsent-waiting').css("display","inherit");$.getJSON('api.php/v1/sentences-with-term',{action:"query",action_type:"example_sentences",lid:lang,word_lc:word,wid:woid,term:word},function(data){$('#exsent-waiting').css("display","none");$('#exsent-sentences').css("display","inherit");const new_element=display_example_sentences(data,ctl);$('#exsent-sentences').append(new_element)})}
function do_ajax_req_sim_terms(lg_id,word_text){return $.getJSON('api.php/v1/similar-terms',{"action":"query","action_type":"similar_terms","simterms_lgid":lg_id,"simterms_word":word_text})}
...
function do_ajax_word_counts(){const t=$('.markcheck').map(function(){return $(this).val()}).get().join(',');$.getJSON('api.php/v1/texts-statistics',{action:"query",action_type:"texts_statistics",texts_id:t},function(data){WORDCOUNTS=data;word_count_click();$('.barchart').removeClass('hide')})}
...
const textid=$('#editimprtextdata').attr('data_id');$.getJSON('api.php/v1/translations',{action:"query",action_type:"term_translations",text_id:textid,term_lc:word},function(data){if("error" in data){alert(data.error)}else{edit_term_ann_translations(data,textid);$.scrollTo(pagepos);$('input.impr-ann-text').on('change',changeImprAnnText);$('input.impr-ann-radio').on('change',changeImprAnnRadio)}})}
